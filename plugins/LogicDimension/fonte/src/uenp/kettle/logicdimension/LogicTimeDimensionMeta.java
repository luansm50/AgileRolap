...
public class LogicTimeDimensionMeta extends BaseStepMeta implements
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans disp) {
		return new LogicDimension(stepMeta, stepDataInterface, cnr, transMeta,
				disp);
	}

	@Override
	public StepDataInterface getStepData() {
		return new LogicDimensionData();
	}
	...
	public void loadXML(Node stepnode, List<DatabaseMeta> databases,
		...
		for (int i = 0; i < hierarquias.size(); i++) {
			...
			for (int j = 0; j < levels.size(); j++) {
				...
				l.getType()
				.loadXML(
						XMLHandler.getSubNode(levels.get(j), "type",
								"value"));
				l.getVisible()
						.loadXML(
