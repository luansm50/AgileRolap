 ...
 * @since 27-05-2003
 */
public class TableROLAP extends Composite 
{
	...
	private Menu mRow;
	private SelectionAdapter lsRowInsBef, lsRowInsAft, lsClipAll, lsCol1, lsCol2, lsRowUp, lsRowDown, lsClear, lsCopyToAll, lsSelAll, lsUnselAll, lsPasteAll, lsCutAll, lsDelAll, lsKeep, lsFilter;
  ...
  private Font       gridFont;
    ...
    private DatabaseMeta inf;
    private TransMeta transMeta;
    private LoggingObjectInterface logging;
    private String schemaTable;
    
	public TableROLAP(VariableSpace space, Composite parent, int style, ColumnInfo[] columnInfo, int nrRows, ModifyListener lsm, PropsUI pr, int[] action)
	{
		this(space, parent, style, columnInfo, nrRows, false, lsm, pr, action);
	}

	public TableROLAP(VariableSpace space, Composite parent, int style, ColumnInfo[] columnInfo, int nrRows, boolean readOnly, ModifyListener lsm, PropsUI pr, int[] action)
	{
		...
		lsUndo = new ModifyListener()
			...
			;
		if(TableROLAP.dummy_gc == null){
      Display disp = parent.getDisplay();
      TableROLAP.dummy_image = new Image(disp, 1, 1);
      TableROLAP.dummy_gc = new GC(TableROLAP.dummy_image);
        
      gridFont = new Font(disp, props.getGridFont());
      TableROLAP.dummy_gc.setFont(gridFont);
		...
		new SelectionAdapter() { public void widgetSelected(SelectionEvent e) { undoAction();       } };
		new SelectionAdapter() { public void widgetSelected(SelectionEvent e) { redoAction();       } };
        
		for(int i = 0; i < action.length; i++){
			switch (action[i]) {
			case Variables.miRowInsBef:
				MenuItem miRowInsBef = new MenuItem(mRow, SWT.NONE); miRowInsBef.setText("TableView.menu.InsertBeforeRow");
				miRowInsBef.addSelectionListener(lsRowInsBef);
				break;
			case Variables.miRowInsAft:
				MenuItem miRowInsAft = new MenuItem(mRow, SWT.NONE); miRowInsAft.setText("TableView.menu.InsertAfterRow");
				miRowInsAft.addSelectionListener(lsRowInsAft);
				break;
			case Variables.miCol1:
				MenuItem miCol1      = new MenuItem(mRow, SWT.NONE); miCol1     .setText("TableView.menu.OptimalSizeWithHeader");
				miCol1     .addSelectionListener(lsCol1 );
				break;
			case Variables.miCol2:
				MenuItem miCol2      = new MenuItem(mRow, SWT.NONE); miCol2     .setText("TableView.menu.OptimalSizeWithoutHeader");
				miCol2     .addSelectionListener(lsCol2 );
				break;
			case Variables.miRowUp:
				MenuItem miRowUp     = new MenuItem(mRow, SWT.NONE); miRowUp    .setText("Mover para Cima");
				miRowUp    .addSelectionListener(lsRowUp );
				break;
			case Variables.miRowDown:
				MenuItem miRowDown   = new MenuItem(mRow, SWT.NONE); miRowDown  .setText("Mover para Baixo");
				miRowDown  .addSelectionListener(lsRowDown );
				break;
			case Variables.miClear:
				MenuItem miClear     = new MenuItem(mRow, SWT.NONE); miClear    .setText("TableView.menu.ClearAll");
				miClear    .addSelectionListener(lsClear );
				break;
			case Variables.miClipAll:
				MenuItem miClipAll   = new MenuItem(mRow, SWT.NONE); miClipAll  .setText("TableView.menu.CopyToClipboard");
				miClipAll  .addSelectionListener(lsClipAll );
				break;
			case Variables.miCopyToAll:
				MenuItem miCopyToAll = new MenuItem(mRow, SWT.NONE); miCopyToAll.setText("TableView.menu.CopyFieldToAllRows");
				miCopyToAll.addSelectionListener(lsCopyToAll);
				break;
			case Variables.miSelAll:
				MenuItem miSelAll    = new MenuItem(mRow, SWT.NONE); miSelAll   .setText("TableView.menu.SelectAll");
				miSelAll   .addSelectionListener(lsSelAll);
				break;
			case Variables.miUnselAll:
				MenuItem miUnselAll  = new MenuItem(mRow, SWT.NONE); miUnselAll .setText("TableView.menu.ClearSelection");
				miUnselAll .addSelectionListener(lsUnselAll);
				break;
			case Variables.miPasteAll:
				MenuItem miPasteAll  = new MenuItem(mRow, SWT.NONE); miPasteAll .setText("TableView.menu.PasteFromClipboard");
				miPasteAll .addSelectionListener(lsPasteAll);
				break;
			case Variables.miCutAll:
				MenuItem miCutAll    = new MenuItem(mRow, SWT.NONE); miCutAll   .setText("TableView.menu.CutSelected");
				miCutAll   .addSelectionListener(lsCutAll);
				break;
			case Variables.miDelAll:
				MenuItem miDelAll    = new MenuItem(mRow, SWT.NONE); miDelAll   .setText("Deletar Linha(s) Selecionada(s)");
				miDelAll   .addSelectionListener(lsDelAll);
				break;
			case Variables.miKeep:
				MenuItem miKeep      = new MenuItem(mRow, SWT.NONE); miKeep     .setText("TableView.menu.KeepSelected");
				miKeep     .addSelectionListener(lsKeep);
				break;
			case Variables.miFilter:
				MenuItem miFilter    = new MenuItem(mRow, SWT.NONE); miFilter   .setText("TableView.menu.FilteredSelection");
	...
	private int[] moveRowsDown(int selectionIndicies[]) {
		...
		for (int i=selectionIndicies.length-1;i>=0;i--) {
			...
			table.getItem(row).setText(3, "");
			table.getItem(newRow).setText(3, "");
		}
	...
	private void delSelected()
	{
		...
		for(int i = 0; i < items.length; i++){
			if(table.getItemCount() > (items[i]+1)){
				table.getItem(items[i]+1).setText(3, "");
			}
		}
	...
	private void setColumnWidthBasedOnTextField(final int colnr, final boolean useVariables)
    {
        ...
        int strmax = TableROLAP.dummy_gc.textExtent(str, SWT.DRAW_TAB | SWT.DRAW_DELIMITER).x+20;
	...
	public void optWidth(boolean header, int nrLines)
	{
		for (int c=0;c<table.getColumnCount();c++)
		{
			...
			if (header) 
			{
				max=TableROLAP.dummy_gc.textExtent(tc.getText(), SWT.DRAW_TAB | SWT.DRAW_DELIMITER).x;
			...
			for (int r=0;r<table.getItemCount() && ( r<nrLines || nrLines<=0);r++)
			{
                ...
                if (ti!=null)
                {
    				...
    				if (str==null) str="";
    				int len = TableROLAP.dummy_gc.textExtent(str, SWT.DRAW_TAB | SWT.DRAW_DELIMITER).x;
    				if (len>max) max=len;
                }
			}
			try
	...
	private void setUndoMenu()
	{
		...
		if (prev!=null) 
		{
			...
			miEditUndo.setText("TableView.menu.Undo");
		} 
		else            
		{
			miEditUndo.setEnabled(false);
			miEditUndo.setText("TableView.menu.UndoNotAvailable");
		} 

		if (next!=null) 
		{
			miEditRedo.setEnabled(true);
			miEditRedo.setText("TableView.menu.Redo");
		} 
		else            
		{
			miEditRedo.setEnabled(false);
			miEditRedo.setText("TableView.menu.RedoNotAvailable");
		} 
	...
	public void setImage(Image image, int colnr, int rownr){
		TableItem item = table.getItem(rownr);
		item.setImage(image);
	}
	...
	public void setConexao(DatabaseMeta inf, TransMeta trans, LoggingObjectInterface l, String schema) {
		this.inf = inf;
	...
	public void changeCombo(){
        ...
        int colnr = activeTableColumn;
        int rownr = table.indexOf(row);
		Database db = new Database(logging, inf);
		db.shareVariablesWith(transMeta);
		if(row.getText(1) != "" && row.getText(1) != null){
			try {
				if(colnr == 2){
					sql = "SELECT *"+Const.CR+"FROM "+inf.getQuotedSchemaTableCombination(schemaTable, row.getText(1))+Const.CR;
					db.connect();
					RowMetaInterface fields = db.getQueryFields(sql, false);
					String op[] = new String[fields.size()];
					if(fields!=null){
				...
				if(colnr == 3 && rownr != 0){
					TableItem rowlast = table.getItem(rownr -1);
					row.setText(3, rowlast.getText(1));
				}
				
				if(colnr == 4 && rownr != 0){
					TableItem rowlast = table.getItem(rownr-1);
					sql = "SELECT *"+Const.CR+"FROM "+inf.getQuotedSchemaTableCombination(schemaTable, rowlast.getText(1))+Const.CR;
					db.connect();
					RowMetaInterface fields = db.getQueryFields(sql, false);
					String op[] = new String[fields.size()];
					if(fields!=null){
